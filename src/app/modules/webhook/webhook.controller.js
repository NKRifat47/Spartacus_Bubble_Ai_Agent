...
export const webhookController = {
  sendgridInbound: async (req, res, next) => {
    try {
      ...
      let aiText = "Thank you for your message. We have received your inquiry and will get back to you shortly.";
      ...
      if (envVars.AI_ENGINE_URL) {
        try {
          ...
          let rawAiText = aiResponse?.data?.reply || aiResponse?.data?.message || aiResponse?.data?.response || aiResponse?.data;
          
          if (typeof rawAiText === "string" && rawAiText.trim() !== "") {
            // Cleanup any stray placeholders
            aiText = rawAiText.replace(/\[Your Name\]/gi, "Spartacus Support")
          ...
          console.error("Failed to get response from AI Engine for inbound email:", err?.message || err);
        }
      }
